...
static inline int deq_schedule(struct cr *cr, job_t func, void *args)
{
    struct task_struct *new_task;
    ...
    if (!new_task)
        return -ENOMEM;
    if (deq_enqueue(&cr->dq, new_task) < 0){
        free(new_task);
        return -ENOMEM;
    }
...
static inline struct task_struct *deq_pick_next_task(struct cr *cr)
{
    return deq_dequeue(&cr->dq);
}

static inline int deq_put_prev_task(struct cr *cr, struct task_struct *prev)
{
    return deq_enqueue(&cr->dq, prev);
}
...
void sched_init(struct cr *cr)
{
    ...
    case CR_MINE:
        deq_init(&cr->dq);
        cr->schedule = deq_schedule;
        cr->pick_next_task = deq_pick_next_task;
        cr->put_prev_task = deq_put_prev_task;
    }
}
